...
// InvoiceSummaries_QuickSelector.aspx page.  The Row or RecordControl classes are the 
...
namespace IDE.UI.Controls.InvoiceSummaries_QuickSelector
{
...
// Base class for the SelectorTableControlRow control on the InvoiceSummaries_QuickSelector page.
...
// Base class for the SelectorTableControl control on the InvoiceSummaries_QuickSelector page.
    ...
    // Setup default pagination settings.
       ...
       // Setup the sorting events.
        ...
        {    
            ...
            {
                BaseColumn col = InvoiceSummariesTable.GetColumnByName(field);
            ...
            if (includeFormulaAndDFKAParameter) 
            {
                if (formula != "")
                {
                    foreach (BaseColumn col in MiscUtils.GetFormulaFieldList(formula, InvoiceSummariesTable.Instance.TableDefinition))
                    {
                        if (col != null && !columnList.Contains(col))
                            columnList.Add(col);             
                    }
                ...
                {
                    BaseColumn col = InvoiceSummariesTable.GetColumnByName(displayFieldName);
                    if (col != null && !columnList.Contains(col))
                        columnList.Add(col);             
                }
            }
      ...
      foreach(BaseColumn col in cols)
      {
          ...
          search.iOR(col, BaseFilter.ComparisonOperator.Contains, MiscUtils.GetSelectedValue(this.Search, this.GetFromSession(this.Search)), true, false);
		        ...
		        if (!string.IsNullOrEmpty(this.Page.Request["dependOnField"]) && this.Page.Request["dependOnVal"] != null) {
			          BaseColumn dependOnCol = InvoiceSummariesTable.GetColumnByName(this.Page.GetDecryptedURLParameter("dependOnField"));
                if (dependOnCol != null){                
...
#region "Helper Functions"
        ...
        public virtual SelectorTableControlRow[] GetSelectedRecordControls()
        {
            ...
            return (SelectorTableControlRow[])((new ArrayList()).ToArray(Type.GetType("IDE.UI.Controls.InvoiceSummaries_QuickSelector.SelectorTableControlRow")));
