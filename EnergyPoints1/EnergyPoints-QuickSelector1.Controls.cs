...
// EnergyPoints_QuickSelector1.aspx page.  The Row or RecordControl classes are the 
...
namespace IDE.UI.Controls.EnergyPoints_QuickSelector1
{
...
// Base class for the SelectorTableControlRow control on the EnergyPoints_QuickSelector1 page.
...
// Base class for the SelectorTableControl control on the EnergyPoints_QuickSelector1 page.
    ...
    // Setup default pagination settings.
       ...
       // Setup the sorting events.
        ...
        {    
            ...
            {
                BaseColumn col = EnergyPoints1Table.GetColumnByName(field);
            ...
            if (includeFormulaAndDFKAParameter) 
            {
                if (formula != "")
                {
                    foreach (BaseColumn col in MiscUtils.GetFormulaFieldList(formula, EnergyPoints1Table.Instance.TableDefinition))
                    {
                        if (col != null && !columnList.Contains(col))
                            columnList.Add(col);             
                    }
                ...
                {
                    BaseColumn col = EnergyPoints1Table.GetColumnByName(displayFieldName);
                    if (col != null && !columnList.Contains(col))
                        columnList.Add(col);             
                }
            }
      ...
      foreach(BaseColumn col in cols)
      {
          ...
          search.iOR(col, BaseFilter.ComparisonOperator.Contains, MiscUtils.GetSelectedValue(this.Search, this.GetFromSession(this.Search)), true, false);
		        ...
		        if (!string.IsNullOrEmpty(this.Page.Request["dependOnField"]) && this.Page.Request["dependOnVal"] != null) {
			          BaseColumn dependOnCol = EnergyPoints1Table.GetColumnByName(this.Page.GetDecryptedURLParameter("dependOnField"));
                if (dependOnCol != null){                
...
#region "Helper Functions"
        ...
        public virtual SelectorTableControlRow[] GetSelectedRecordControls()
        {
            ...
            return (SelectorTableControlRow[])((new ArrayList()).ToArray(Type.GetType("IDE.UI.Controls.EnergyPoints_QuickSelector1.SelectorTableControlRow")));
