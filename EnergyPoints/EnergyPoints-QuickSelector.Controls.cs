...
// EnergyPoints_QuickSelector.aspx page.  The Row or RecordControl classes are the 
...
namespace IDE.UI.Controls.EnergyPoints_QuickSelector
{
...
// Base class for the SelectorTableControlRow control on the EnergyPoints_QuickSelector page.
...
// Base class for the SelectorTableControl control on the EnergyPoints_QuickSelector page.
    ...
    // Setup default pagination settings.
       ...
       // Setup the sorting events.
        ...
        {    
            ...
            {
                BaseColumn col = EnergyPointsTable.GetColumnByName(field);
            ...
            if (includeFormulaAndDFKAParameter) 
            {
                if (formula != "")
                {
                    foreach (BaseColumn col in MiscUtils.GetFormulaFieldList(formula, EnergyPointsTable.Instance.TableDefinition))
                    {
                        if (col != null && !columnList.Contains(col))
                            columnList.Add(col);             
                    }
                ...
                {
                    BaseColumn col = EnergyPointsTable.GetColumnByName(displayFieldName);
                    if (col != null && !columnList.Contains(col))
                        columnList.Add(col);             
                }
            }
      ...
      foreach(BaseColumn col in cols)
      {
          ...
          search.iOR(col, BaseFilter.ComparisonOperator.Contains, MiscUtils.GetSelectedValue(this.Search, this.GetFromSession(this.Search)), true, false);
		        ...
		        if (!string.IsNullOrEmpty(this.Page.Request["dependOnField"]) && this.Page.Request["dependOnVal"] != null) {
			          BaseColumn dependOnCol = EnergyPointsTable.GetColumnByName(this.Page.GetDecryptedURLParameter("dependOnField"));
                if (dependOnCol != null){                
      ...
      foreach(BaseColumn col in cols)
      {
        ...
        public virtual string[] GetAutoCompletionList_Search(String prefixText,int count)
        {
            ...
            if(count==0) count = 10;
            IDE.Business.EnergyPointsRecord[] recordList  = EnergyPointsTable.GetRecords(filterJoin, wc, null, 0, count, ref count);
            String resultItem = "";
            if (resultItem == "") resultItem = "";
            foreach (EnergyPointsRecord rec in recordList ){
...
#region "Helper Functions"
        ...
        public virtual SelectorTableControlRow[] GetSelectedRecordControls()
        {
            ...
            return (SelectorTableControlRow[])((new ArrayList()).ToArray(Type.GetType("IDE.UI.Controls.EnergyPoints_QuickSelector.SelectorTableControlRow")));
